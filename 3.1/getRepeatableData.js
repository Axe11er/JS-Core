/**
 * TODO Написать функцию getRepeatableData, котрая принимает на вход три параметра:
...
Если вызов getData возвращает ошибку TemporaryError, то мы должны делать повторный вызов getData функции. Кол-во таких вызовов не должно превышать значение maxRequestsNumber. Если кол-во повторных вызовов превышает maxRequestsNumber, то функция getRepeatableData должна пробрасывать ошибку AttemtsLimitExceeded.
...
const getRepeatableData = (getData, key, maxRequestsNumber = Infinity) => {
   if (!maxRequestsNumber) throw new AttemptsLimitExceeded();
   try {
      getData(key);
   } catch (error) {
...
const getData = key => 'hello' + key;
const res = getRepeatableData(getData, '1', 5); // 'hello1'
console.log(res);
